...
function applyFilters() {
    ...
    displayMembers(member => {
        return (!ageFilter || member.age === ageFilter) &&
               (!courseFilter || member.course.toLowerCase().includes(courseFilter)) &&
...
function addMember() {
    ...
    if (photoInput) {
        ...
        reader.onload = function(event) {
            ...
            if (pdfInput) {
                ...
                pdfReader.onload = function(event) {
                    ...
                    saveMember(name, age, skills, experience, phone, email, course, linkedin, photoUrl, pdfUrl);
                };
                pdfReader.readAsDataURL(pdfInput);
            } else {
                saveMember(name, age, skills, experience, phone, email, course, linkedin, photoUrl, null);
            }
        };
        ...
        pdfReader.onload = function(event) {
            const pdfUrl = event.target.result;
            saveMember(name, age, skills, experience, phone, email, course, linkedin, null, pdfUrl);
        };
        pdfReader.readAsDataURL(pdfInput);
    } else {
        saveMember(name, age, skills, experience, phone, email, course, linkedin, null, null);
    }
}

function saveMember(name, age, skills, experience, phone, email, course, linkedin, photoUrl, pdfUrl) {
